...
par <- list(
  ...
  "github_url" = "https://github.com/openproblems-bio/task_denoising/tree/main/"
)
meta <- list(
  "temp_dir" = "temp/"
)
...
cat("Read task info\n")
task_api <- openproblems.docs::read_api_files(par[["task_dir"]])
viash_info <- yaml.load_file(par[["viash_yaml"]])

cli::cli_inform("Reading task authors")
authors <- map_df(viash_info$authors, function(aut) {
  aut$roles <- paste(aut$roles, collapse = ", ")
  openproblems.docs::list_as_tibble(aut)
})

# determine ordering
root <- openproblems.docs:::.task_graph_get_root(task_api)

r_graph <- openproblems.docs::render_task_graph(task_api, root)
...
r_details <- map_chr(
  ...
  function(file_name) {
    if (file_name %in% names(task_api$comp_specs)) {
      openproblems.docs::render_component(task_api$comp_specs[[file_name]])
    } else {
      openproblems.docs::render_file(task_api$file_specs[[file_name]])
    }
  }
)
...
authors_str <-
  if (nrow(authors) > 0) {
    paste0(
      ...
      authors %>% knitr::kable() %>% paste(collapse = "\n"),
      "\n"
    )
  } else {
    ""
...
readme_str <-
  if (is.null(viash_info$info$readme) || is.na(viash_info$info$readme)) {
    paste0(
      "\n## README\n\n",
      readme_base,
      "\n"
    ...
    paste0(
      "\n## README\n\n",
      viash_info$info$readme,
      "\n"
    )
  }

# get relevant task info
